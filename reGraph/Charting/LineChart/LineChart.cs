...
namespace AeoGraphing.Charting.LineChart
{
    public class LineChart : ISizeable
    {
        ...
        private float chartTop => _style.Padding.GetFloatValue(this.Height) + titleHeight + descriptionHeight + (DataSource.HasGrouping ? _style.GroupingNameSpace.GetFloatValue(this.Height) : 0);
        ...
        private void drawDataSeries(Graphics graphics)
        {
        ...
        private void drawDataGroups(Graphics ctx)
        {
            ...
            int index = 0;
            float width = (float)(pixelPerBaseValue * DataSource.GroupingInterval.Value);
            StringFormat stringFormat = new StringFormat() { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center };
            for (double i = 0; i < DataSource.ScaledBaseValue; i += DataSource.GroupingInterval.Value)
            {
                var x = (float)(pixelPerBaseValue * i) + valueLineWidth;
                ctx.DrawLine(pen, new PointF(x, chartTop), new PointF(x, baseLinePos));
                if (DataSource.DataGroupNames.Count > index)
                {
                    var heigth = _style.GroupingNameSpace.GetFloatValue(this.Height);
                    var rect = new RectangleF(x, chartTop - heigth, width, heigth);
                    ctx.DrawString(DataSource.DataGroupNames[index], _style.DataCaptionFont, new SolidBrush(_style.TextColor), rect, stringFormat);
                    index++;
                }
